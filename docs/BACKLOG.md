...
| BUG-05 | A code comment claims a privacy guarantee stronger than the requirement | `types.ts:90` says exact coordinates "never leave the device in the MVP". NFR-04 actually says never *publish* the exact home location, which is a different and weaker claim. The comment reads as a hard constraint and nearly ruled out private per-account sync in spec 005 on a requirement that does not say that. Either the comment is corrected to match NFR-04, or NFR-04 is tightened deliberately, but the two should not disagree silently. | `src/domain/types.ts:90`, `docs/PRD.md:414` |
