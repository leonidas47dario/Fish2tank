...
### 2. FishBase is CC-BY-NC — and that hook has now caught

Non-commercial only, with attribution.

**Settled 2026-09-03: assume commercial.** So FishBase is out. Not "out for
now" or "out pending a conversation" — CC-BY-NC forbids the use this project
is now planning for, and building on it would mean either renegotiating a
licence or tearing it out once species depended on it.

This is the decision being made at the cost it was flagged at. Twice the spec
recorded that asking early was much cheaper than asking after 2,000 species
depended on the answer; asking early is what happened, and the answer removed
the single largest coverage win before a line of ingest code existed. That is
the outcome the flag was for, not a failure of it.
...
2. ~~**FishBase second**, for size, temperature and pH across the ~50%
   ...
   **Removed — see Revision 3.** Wikipedia stays the size source by default,
   and temperature and pH stay thin outside SeriouslyFish's reach.
...
## Revision 3: commercial, so FishBase is out

> yes assume commercial

The one answer that decides whether the biggest source is usable at all, and
it decides against it.

### What changes

**FishBase is removed from the plan entirely.** It was step 2 — size,
temperature and pH across the roughly half of the freshwater catalog
SeriouslyFish does not reach, and the replacement for Wikipedia as the size
source. None of that happens now.

### What does not change

**SeriouslyFish still leads, and commercial use does not alter that.** Its
pages are © all rights reserved either way; what makes the ingest legitimate is
the rule this spec set in its first revision and has not moved from:

> Numbers and measured ranges are not copyrightable; the article text around
> them is.

Taking a minimum volume and a temperature range is as sound commercially as it
was personally. Copying a care write-up was never acceptable and still is not.
The same reasoning carries aqua-fish.net.

So the plan loses its second pass and keeps its first — which is the pass that
carried minimum volume and aggression, the two fields the compatibility engine
actually screens on and the two no other source supplies.

### What it costs, stated rather than glossed

Wikipedia remains the size source. For the ~50% of freshwater fish
SeriouslyFish does not reach, **temperature and pH stay close to where they
are** — 9.6% and 0%. FishBase was going to lift both across that half, and
nothing else on the evaluated list does it at that scale.

The honest reading: this project's care coverage is now **capped by
SeriouslyFish's freshwater catalog** until a commercially usable bulk source
turns up. Finding one is its own piece of work and should not be smuggled into
the ingest.

### And it makes P6 load-bearing rather than decorative

Revision 1 already recorded that no source gives minimum volume and aggression
at scale, so *"not enough data"* had to remain the answer for most species.
Losing FishBase widens that to temperature and pH as well.

That is not a degraded outcome to apologise for. FR-E05 and principle P6 say a
missing fact renders as missing, never as a plausible guess — so the app's
correct behaviour under this constraint is the behaviour it already has. What
would be a failure is filling the gap with a number nobody measured, which is
precisely how the `PLACEHOLDER` problem this spec exists to fix began.

### Revised shape

1. **SeriouslyFish, scoped to the 958 freshwater fish.** Unchanged, and now
   the only bulk pass.
2. ~~FishBase~~ — removed, CC-BY-NC.
3. **aqua-fish.net**, counted before committing, to lift husbandry further.
4. **Retire `PLACEHOLDER` per field** as each is genuinely replaced.
5. **One source per field**, not per profile.

### Still open

- **A commercially usable source for size, temperature and pH at scale.**
  FishBase's slot is empty, not filled. Worth its own evaluation rather than a
  guess; the shape of that evaluation is this document's own source table.
