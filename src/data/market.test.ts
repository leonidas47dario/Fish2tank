...
describe('resolve rates are derived from the index, never hardcoded', () => {
  it('is published listings over livestock listings, per store', () => {
    ...
    for (const s of MARKET_INDEX.sources) {
      // Mirrors the guard in market.ts. A vendor that published no listings
      // has no resolve rate to compute - dividing by its zero yields NaN, not
      // a rate. Petco is the case: its storefront refuses automated clients,
      // so it contributes locations only and fetches nothing.
      const expected = s.listingsFetched > 0 ? (published[s.id] ?? 0) / s.listingsFetched : 0;
      expect(STORE_RESOLVE_RATES[s.id]).toBeCloseTo(expected, 10);
    }
  });
