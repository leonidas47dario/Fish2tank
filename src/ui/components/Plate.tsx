...
export function useCardArt(card: CatalogCard): { art: CardArt; ownUrl?: string } {
  ...
  // Your own photos live in IndexedDB as bytes, so they need an object URL.
  ...
  // One object URL per media id, revoked when it goes out of use. Without this
  // a scroll through the catalog leaks one blob URL per own-photo card.
  useEffect(() => () => { if (ownUrl) URL.revokeObjectURL(ownUrl); }, [ownUrl]);
